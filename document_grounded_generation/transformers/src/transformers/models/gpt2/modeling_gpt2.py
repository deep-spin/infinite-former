...
from .configuration_gpt2_inftyformer import GPT2ConfigInftyFormer
from .configuration_gpt2_inftyformer_sticky import GPT2ConfigInftyFormerSticky
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False, layer_n=None):
    ...
    def forward(
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask, output_attentions, layer_n=layer_n)
        ...
        if self.use_long_term_attention and (mem is not None or generating):
            if self.kl_regularizer:
                a_long_term, kl_reg = self.long_term_attention(mem, query, new_doc, generating, layer_n=layer_n)
            else:
                a_long_term = self.long_term_attention(mem, query, new_doc, generating, layer_n=layer_n)
    ...
    def forward(
        ...
        if self.long_term_attention:
            if last_doc: 
                ...
                self.mem = self.ln_1(hidden_states.detach())
...
class GPT2PreTrainedModel(PreTrainedModel):
    ...
    #config_class = GPT2ConfigInftyFormer
    #config_class = GPT2ConfigInftyFormerSticky
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.long_term_attention and self.kl_regularizer:
            transformer_outputs, kl_regs = self.transformer(
                input_ids,
