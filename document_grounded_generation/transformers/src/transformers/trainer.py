...
#logging.basicConfig(filename='log',filemode='a',datefmt='%H:%M:%S')
...
class Trainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if n in decay_parameters and 'long_term_attention' not in n and "compress" not in n],
                    "weight_decay": self.args.weight_decay,"lr": self.args.learning_rate
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if n not in decay_parameters and 'long_term_attention' not in n and "compress" not in n],
                    "weight_decay": 0.0,"lr": self.args.learning_rate
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if 'long_term_attention' in n or "compress" in n],
                    "weight_decay": 0.0,"lr": self.args.learning_rate_ltm
                }
            ]
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if self.kl_regularizer:
                        tr_loss_, kl_loss_ = self.training_step(model, inputs)
                        
                        tr_loss+=tr_loss_
                        if kl_loss_ is not None:
                            kl_loss.append(kl_loss_)
                    elif self.compressive:
                        tr_loss_, comp_loss_ = self.training_step(model, inputs)
                        
                        tr_loss+=tr_loss_
                        if comp_loss_ is not None:
                            comp_loss.append(comp_loss_)
                    else:
                        tr_loss += self.training_step(model, inputs)
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.kl_regularizer:
                        self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, kl_loss=kl_loss)
                    elif self.compressive:
                        self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, comp_loss=comp_loss)
                    else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, kl_loss=None, comp_loss=None):
        if self.control.should_log:
            ...
            if kl_loss is not None:
                kl_loss_scalar = torch.FloatTensor(kl_loss).mean().item()
                logs["kl reg"] = round(kl_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
                kl_loss = []
            if comp_loss is not None:
                comp_loss_scalar = torch.FloatTensor(comp_loss).mean().item()
                logs["comp loss"] = round(comp_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
                comp_loss = []
        ...
        if self.control.should_save:
            self.log_file.write('step: '+str(self.state.global_step)+'\n')
            self.log_file.write('perplexity: '+str(metrics["perplexity"])+'\n')
            if metrics["perplexity"]<self.best_perplexity:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.use_amp:
            ...
            if self.kl_regularizer and kl_reg is not None:
                kl_reg = kl_reg.mean()
                (loss + self.kl_m*kl_reg).backward()
            elif self.compressive and c_loss is not None:
                c_loss = c_loss.mean()
                (loss+c_loss).backward()
            else:
                loss.backward()
        if self.kl_regularizer and kl_reg is not None:
            return loss.detach(), kl_reg.detach()
        elif self.kl_regularizer:
            return loss.detach(), kl_reg
        elif self.compressive and c_loss is not None:
            return loss.detach(), c_loss.detach()
        elif self.compressive:
            return loss.detach(), c_loss
        else:
            return loss.detach()
    ...
    def prediction_loop(
        ...
        if eval_loss is not None:
            ...
            metrics["perplexity"] = np.exp(eval_loss.mean()).item()

        # Prefix all keys with metric_key_prefix + '_'
        for key in list(metrics.keys()):
            if not key.startswith(f"{metric_key_prefix}_") and key != "perplexity":
